...
public class AiVoiceOverGenerator {
    ...
    public List<VoiceModelDescriptor> listVoiceModels(AiProvider provider) {
        AiProvider resolved = provider != null ? provider : defaultProvider;
        TtsClient client = ttsClients.get(resolved);
        if (client == null) {
            throw new IllegalArgumentException("No TTS client registered for provider: " + resolved);
        }
        return client.listVoiceModels();
    }
}